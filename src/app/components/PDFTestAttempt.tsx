...
function FormattedHeader({ text, isAttempt = false }: { text: string; isAttempt?: boolean }) {
  ...
  if (isStatements) {
    // FIX: Unique regex that doesn't dupe split bold markers
    const parts = text.split(/(?=(?:\*\*)*Statement\s+[IVX]+)/gi);
    const intro = (parts[0] || "").trim();
    
    const statementItems = parts.slice(1);
    const processed = statementItems.map(item => {
        // Robust strip that handles colons and bolding
        const cleaned = item.replace(/^(\*\*)?Statement\s+[IVX]+(\*\*)*\s*[:]*\s*/i, '').trim();
        return cleaned;
    }).filter(item => item.length > 2); // Increased threshold to avoid small fragments
