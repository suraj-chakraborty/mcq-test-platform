...
export default function TakeTestPage() {
  ...
  // Extract Comprehension/Passage from Question Text if available
  ...
  const stats = useMemo(() => {
    if (!test) return { answered: 0, unattempted: 0, marked: 0, notAnswered: 0 };
    let answered = 0;
    let marked = 0;
    let notAnswered = 0;
    let unattempted = 0;
  ...
  }, [test, answers, markedForReview, visited]);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-neutral-950 p-4">
        <div className="text-center p-8 bg-white dark:bg-neutral-900 rounded-2xl border border-gray-200 dark:border-neutral-800 shadow-sm max-w-md">
          <h2 className="text-lg font-black text-gray-800 dark:text-white mb-2">Test Not Found</h2>
          <p className="text-xs text-gray-500 mb-6">The requested test could not be loaded or has expired.</p>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-100 dark:bg-neutral-950 text-gray-900 dark:text-white select-none">
        ...
        <div className="max-w-[1700px] mx-auto px-3 sm:px-6 h-13 sm:h-14 flex items-center justify-between gap-2 sm:gap-4">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5 shrink-0">
            ...
            <Button
              ...
              className="h-8 px-3 sm:px-4 rounded-lg bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-700 hover:to-violet-700 text-white font-black text-xs uppercase tracking-wider shadow-sm shrink-0"
            >
              Submit
            </Button>
          </div>
      ...
      {/* 2. SUB-HEADER: QUESTION STATUS & ACTIONS */}
        ...
        <div className="max-w-[1700px] mx-auto flex items-center justify-between gap-2 text-xs font-medium">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            ...
            <button
              ...
              className="px-2 sm:px-2.5 py-1 rounded-lg flex items-center gap-1 text-[11px] sm:text-xs font-bold text-gray-500 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/40 transition-colors"
            >
        ...
        <div className="flex-1 flex flex-col bg-white dark:bg-neutral-900 rounded-xl sm:rounded-2xl border border-gray-200 dark:border-neutral-800 shadow-sm overflow-hidden">
          <div className="flex-1 overflow-y-auto p-4 sm:p-7 space-y-4 sm:space-y-6">
            <div className={`grid ${comprehension ? 'grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6' : 'grid-cols-1'} items-start`}>
              {/* Left Column: Comprehension Passage (if present) */}
              {comprehension && (
                <div className="space-y-2 sm:space-y-3 lg:border-r lg:border-gray-200 dark:lg:border-neutral-800 lg:pr-6">
                  ...
                  <div className="text-xs sm:text-sm text-gray-800 dark:text-gray-200 leading-relaxed font-normal whitespace-pre-line bg-gray-50/70 dark:bg-neutral-800/40 p-3.5 sm:p-4 rounded-xl border border-gray-200/60 dark:border-neutral-700/60">
                    {comprehension}
                  </div>
                </div>
              )}

              {/* Right/Center Column: Question & Options */}
              <div className="space-y-4 sm:space-y-6">
                ...
                {/* 4 Clean Options with active selection states */}
                ...
                {/* Proof Citation Badge if available */}
        ...
        >
          {sidebarCollapsed ? (
            ...
            <div className="flex-1 flex flex-col space-y-3.5 overflow-y-auto">
              ...
              {/* 5-Column Question Grid - COMPACT & SMALL TILES */}
      ...
      <Dialog open={showSubmitModal} onOpenChange={setShowSubmitModal}>
        <DialogContent className="max-w-md rounded-xl p-6 bg-white dark:bg-neutral-900 border border-gray-200 dark:border-neutral-800">
          ...
          <div className="grid grid-cols-2 gap-3 py-3 text-xs font-bold">
            <div className="p-3 bg-emerald-50 dark:bg-emerald-950/40 rounded-lg border border-emerald-100 dark:border-emerald-900/40 text-emerald-800 dark:text-emerald-200">
              <span className="text-lg block font-black">{stats.answered}</span>
              <span>Answered</span>
            </div>
            <div className="p-3 bg-rose-50 dark:bg-rose-950/40 rounded-lg border border-rose-100 dark:border-rose-900/40 text-rose-800 dark:text-rose-200">
